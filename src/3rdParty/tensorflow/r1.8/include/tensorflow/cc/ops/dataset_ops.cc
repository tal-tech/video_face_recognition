...
#include "D:/work/lib/tensorflow/tensorflow/contrib/cmake/build/tensorflow/cc/ops/dataset_ops.h"
...
IteratorSetStatsAggregator::IteratorSetStatsAggregator(const
                                                       ::tensorflow::Scope&
                                                       scope,
                                                       ::tensorflow::Input
                                                       iterator_handle,
  ...
  if (!scope.ok()) return;
  auto _iterator_handle = ::tensorflow::ops::AsNodeOut(scope, iterator_handle);
  if (!scope.ok()) return;
  auto _stats_aggregator_handle = ::tensorflow::ops::AsNodeOut(scope, stats_aggregator_handle);
  if (!scope.ok()) return;
  ::tensorflow::Node* ret;
  const auto unique_name = scope.GetUniqueNameForOp("IteratorSetStatsAggregator");
  auto builder = ::tensorflow::NodeBuilder(unique_name, "IteratorSetStatsAggregator")
                     .Input(_iterator_handle)
                     .Input(_stats_aggregator_handle)
  ;
  scope.UpdateBuilder(&builder);
  scope.UpdateStatus(builder.Finalize(scope.graph(), &ret));
  if (!scope.ok()) return;
