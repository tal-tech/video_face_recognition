...
#include "D:/work/lib/tensorflow/tensorflow/contrib/cmake/build/tensorflow/cc/ops/bitwise_ops.h"
...
LeftShift::LeftShift(const ::tensorflow::Scope& scope, ::tensorflow::Input x,
                     ::tensorflow::Input y) {
  if (!scope.ok()) return;
  auto _x = ::tensorflow::ops::AsNodeOut(scope, x);
  if (!scope.ok()) return;
...
RightShift::RightShift(const ::tensorflow::Scope& scope, ::tensorflow::Input x,
                       ::tensorflow::Input y) {
  if (!scope.ok()) return;
  auto _x = ::tensorflow::ops::AsNodeOut(scope, x);
  if (!scope.ok()) return;
