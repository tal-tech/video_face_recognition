...
void FileDecrypt(const std::string& proto_file, std::string* txt_string, std::string* binary_string, int key, const int len_max)
{
	...
	if (len_file == 0)
	{
		...
		std::cout<<"Failed to read model data: " << proto_file << " " << len_file << std::endl;
		return;
	}
	...
	if (len > len_file || len <= 0)
	{
		...
		std::cout<<"Failed to read model size: " << len << " " << len_file << std::endl;
		return;
	}
...
Status ReadBinProto(tensorflow::Env * env, const string& fname, tensorflow::protobuf::MessageLite* proto, int key, int len_max) {

	std::string binary_string;
	std::string txt_string;

	FileDecrypt(fname, &txt_string, &binary_string, key, len_max);
	if(binary_string.length() == 0){
		return tensorflow::errors::DataLoss("Can't parse ", fname, " as binary proto");
	}

	tensorflow::protobuf::io::CodedInputStream* coded_stream = new tensorflow::protobuf::io::CodedInputStream((unsigned char*)binary_string.data(), binary_string.length());
	// Total bytes hard limit / warning limit are set to 1GB and 512MB
	// respectively.
	coded_stream->SetTotalBytesLimit(1024LL << 20, 512LL << 20);
	bool sucess = proto->ParseFromCodedStream(coded_stream);
	delete coded_stream;
