...
namespace facethink {
    ...
    public:
		...
		*/
		...
		/// @return
		EXPORT_CLASS_API virtual void detection_SCALE(const cv::Mat& img, std::vector<cv::Rect>& rectangles, std::vector<float>& confidence, bool is_rgb_format = false) = 0;
		...
		/// @param [in] is_rgb_format，默认值为false时，表示图片格式为BGR。
		/// @return
		EXPORT_CLASS_API virtual void detection_SCALE(const cv::Mat& img, std::vector<cv::Rect>& rectangles, std::vector<float>& confidence, std::vector<std::vector<cv::Point>>& alignment, bool is_rgb_format = false) = 0;
